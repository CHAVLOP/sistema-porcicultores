...
define("PASSWORD_PREDETERMINADA", "NuevoUsuario");
date_default_timezone_set('America/Mexico_City');
define("HOY", date("Y-m-d"));



function obtenerProductosPersonalizada($consultaSQL) {
...
function obtenerContrasenaAdmin($usuario_id) {
    ...
    $sql = "SELECT admin_contra FROM usuarios WHERE id = ?";

    // Prepara la consulta
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $usuario_id); // "i" indica un entero

    // Ejecuta la consulta
    $stmt->execute();

    // Obtiene el resultado
    $stmt->bind_result($contrasena_admin);
    $stmt->fetch();

    // Cierra la conexión y devuelve la contraseña del administrador
    $stmt->close();
    $conn->close();
...
/**
 * Obtiene los productos que fueron ingresados en un rango de fechas.
 *
 * @param string|null $nombreProducto Nombre o código del producto a buscar (opcional).
 * @param string|null $fechaInicio    Fecha de inicio del rango (opcional).
 * @param string|null $fechaFin       Fecha de fin del rango (opcional).
 * @return array                     Array de objetos que representan los productos encontrados.
 */
function obtenerProductosPorFechas($nombreProducto = null, $fechaInicio = null, $fechaFin = null) {
    ...
    try {
        ...
        if (!empty($nombreProducto)) {
            $query .= " AND (nombre LIKE :nombreProducto OR codigo LIKE :nombreProducto)";
        }

        if (!empty($fechaInicio) && !empty($fechaFin)) {
            $query .= " AND fecha_registro BETWEEN :fechaInicio AND :fechaFin";
        }

        // Preparar la consulta
        $stmt = $db->prepare($query);

        // Asignar valores a los marcadores de posición si se proporcionaron parámetros
        if (!empty($nombreProducto)) {
        ...
        // También puedes registrar el error en un archivo de registro si lo deseas
...
function obtenerClientesConTipo() {
    $pdo = conectarBaseDatos();
    $consulta = "SELECT c.id, c.nombre, c.telefono, c.direccion, c.deuda, c.credito, tc.tipos AS tipo_cliente
                FROM clientes c
                JOIN tipo_cliente tc ON c.tipo = tc.id_tipo";
...
function obtenerTiposDeCliente() {
    // Realiza la conexión a la base de datos usando PDO
    $pdo = conectarBaseDatos();

    // Consulta los tipos de cliente desde la tabla "tipo_cliente"
    $consulta = "SELECT id_tipo, tipos FROM tipo_cliente";
    $stmt = $pdo->prepare($consulta);
    $stmt->execute();

    // Obtiene los resultados como un array asociativo
    $tiposDeCliente = $stmt->fetchAll(PDO::FETCH_ASSOC);

    // Cierra la conexión a la base de datos
    $pdo = null;

    // Retorna el array de tipos de cliente
...
function editarCliente($nombre, $telefono, $direccion, $tipo_cliente, $deuda, $credito, $id) {
    // Elimina las comas de los valores de deuda y crédito
    $deuda = str_replace(',', '', $deuda);
    $credito = str_replace(',', '', $credito);

    $sentencia = "UPDATE clientes SET nombre = ?, telefono = ?, direccion = ?, tipo = ?, deuda = ?, credito = ? WHERE id = ?";
    $parametros = [$nombre, $telefono, $direccion, $tipo_cliente, $deuda, $credito, $id];
    return editar($sentencia, $parametros);
}
...
function registrarCliente($nombre, $telefono, $tipo_cliente, $deuda, $credito, $direccion){
    $sentencia = "INSERT INTO clientes (nombre, telefono, tipo, deuda, credito, direccion) VALUES (?,?,?,?,?,?)";
    $parametros = [$nombre, $telefono, $tipo_cliente,$deuda, $credito, $direccion];
    return insertar($sentencia, $parametros);
}
...
function registrarProductosVenta($productos, $idVenta){
    ...
    foreach ($productos as $producto) {
        $precioSeleccionado = property_exists($producto, 'precioSeleccionado') ? $producto->precioSeleccionado : 'venta';
        $parametros = [$producto->cantidad, $producto->{$precioSeleccionado}, $producto->id, $idVenta];
...
function calcularTotalLista($lista){
    ...
    foreach ($lista as $producto) {
        if (is_object($producto)) {
            // Verifica si la propiedad precioSeleccionado existe en el objeto $producto
            $precioSeleccionado = property_exists($producto, 'precioSeleccionado') ? $producto->precioSeleccionado : 'venta';
            ...
            switch ($precioSeleccionado) {
                ...
                case "precio_tonelada":
                    $precio = $producto->precio_tonelada;
                    break;
                default:
                    $precio = 0;
            }

            $subtotal = floatval($precio) * floatval($producto->cantidad);
            $total += $subtotal;
        }
    }
    return $total;
...
function obtenerProductoPorNombre($nombre) {
    ...
    $producto = select($sentencia, [$nombre]);
    if($producto) return $producto[0];
    return [];
}
...
function contarCategoriasRegistradas() {
    $sentencia = "SELECT COUNT(*) AS total FROM categorias";
    $fila = select($sentencia);
    if ($fila) {
        return $fila[0]->total;
    } else {
        return 0; // En caso de no encontrar ninguna categoría
    }
}
...
function editarProducto($codigo, $nombre, $categoria, $presentacion, $compra, $venta, $kilo, $bulto, $tonelada, $existencia,$fecha_registro, $id){
    $sentencia = "UPDATE productos SET codigo = ?, nombre = ?, categoria = ?, presentacion = ?, compra = ?,  venta = ?, precio_kilo = ?, precio_bulto = ?, precio_tonelada = ?, existencia = ?, fecha_registro = ? WHERE id = ?";
    $parametros = [$codigo, $nombre, $categoria, $presentacion, $compra, $venta, $kilo, $bulto, $tonelada, $existencia,$fecha_registro, $id];
    return editar($sentencia, $parametros);
}
...
function registrarProducto($codigo, $nombre,$categoria, $presentacion, $compra, $venta, $kilo, $bulto, $tonelada, $existencia, $fecha_registro){
    $sentencia = "INSERT INTO productos(codigo, nombre, categoria, presentacion, compra, venta, precio_kilo, precio_bulto, precio_tonelada, existencia, fecha_registro) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
    $parametros = [$codigo, $nombre,$categoria, $presentacion, $compra, $venta, $kilo, $bulto, $tonelada, $existencia, $fecha_registro];
    return insertar($sentencia, $parametros);
}
